 ...
 * @param {object|null} gating - `context.gating` (veto_long/short + veto_reason + contradiction_count).
 ...
 */
export function applyDecisionGates(rawStructured, gating, failsafeEnabled) {
  const vetoActive = !!(gating?.veto_long || gating?.veto_short);
  if (vetoActive) {
    // El veto es autoritativo: imponer gating_active=true ANTES de validar → el validador
    // dispara `gating_forces_wait` (severo) si el LLM no puso Esperar.
    rawStructured.gating_active = true;
    rawStructured.gating_reason = rawStructured.gating_reason ?? gating?.veto_reason ?? null;
  }
  ...
  const hardGate = vetoTriggered || decayTriggered;
