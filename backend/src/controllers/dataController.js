...
/**
 * Parsea `ai_response_full` (JSON string) a su forma persistida; null si falta o es inválido.
 * Acepta DOS esquemas — filas anteriores al pivot a narrador (`{structured, narrative}`) y
 * posteriores (`{narrative, executive_summary}`) — "los escritores dejan de producir, los
 * lectores siguen entendiendo": un análisis viejo no debe dejar de leerse solo porque cambió
 * el esquema de los nuevos.
 */
function parseAiResponseFull(raw) {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw);
    return (parsed?.structured || parsed?.narrative) ? parsed : null;
  } catch {
    return null;
  }
}
  ...
  try {
    ...
    res.json({
      ...
      last_analysis: lastAnalysisRow ? {
        ...
        // GEOMETRÍA DE RIESGO — sustituye al viejo `conditional_plan` de una sola dirección.
        // Ya viene completa y persistida desde el momento del análisis (utils/riskGeometry.js
        // usa el ATR de decisión, síncrono) — no hace falta recomputar nada aquí.
        risk_geometry: lastAnalysisRow.risk_geometry
          ? JSON.parse(lastAnalysisRow.risk_geometry) : null,
      } : null,
