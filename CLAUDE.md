...
`P(disparo)` (`TRIGGER_BASE_RATE`) y `P(objetivo en la vigencia)` (`TARGET_REACHABILITY`, que en
los setups reales sale al **5-18 %** — o sea que lo normal es que el plan caduque abierto). Más
el registro crudo de los planes anteriores **en la misma pantalla**, no en una página aparte.

**Se niega a enseñar** `expectancy_r` (su línea base es una curva sin medir — M10) y cualquier
probabilidad de acierto direccional. `risk_score` **retirado** (su única regla era *"entero en
[1,10]"*); la convicción va etiquetada *"auto-declarada, sin calibrar"*.

**Y no exigió punto cero:** es presentación de cosas que el backend ya calculaba y persistía.
