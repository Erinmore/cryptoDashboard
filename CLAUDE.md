...
src/services/                ← Lógica de negocio, I/O externo, cache
  ...
  historyService.js          ← Históricos por coin para análisis LLM (7-30 días) — en memoria (ventana LLM) + persistencia SQLite write-through (tabla history_series). ⚠️ **`loadSeries` distingue desde el 2026-08-04 «no he podido mirar» de «está vacía»**: `getDb()` LANZA si nadie llamó a `initDb()`, y el `catch` devolvía `[]` con nivel `debug` —invisible en producción—, así que una BBDD no disponible era indistinguible de una serie sin datos. Los dos casos siguen degradando a `[]` (fallar aquí no debe tumbar la app, y en producción no muerde porque `app.js` inicializa antes que nada), pero el primero avisa en **`warn`**. **No era un bug activo; era degradación silenciosa que produce un valor PLAUSIBLE** — y mordió: el diff del payload de B5 pasó por vacío sobre CVD/VWAP creyendo que las series estaban vacías, y sólo se vio comprobando la cobertura a mano. Guarda en `historyHydrationWarning.test.js`, verificada reintroduciendo el defecto. **Por qué sólo CVD/VWAP se hidratan, y por qué NO conviene normalizarlo:** las otras cinco series vienen de APIs que sirven su propio histórico y se refrescan enteras en cada poll, así que hidratarlas sería trabajo pisado segundos después — y peor, mezclaría dos procedencias en una misma serie, que es justo lo que `backfillHistorySeries` ABORTA por diseño (CVD `heuristic` vs `taker_real`) y lo que ya costó caro con las liquidaciones guardadas como USD siendo monedas. CVD/VWAP se hidratan al arrancar (únicas sin API que sirva su histórico ya calculado — pero SÍ reconstruibles con `scripts/backfillHistorySeries.mjs`, ver abajo); el resto se persiste para acumular pero se rellena fresco de su API
  (scripts) auditVetoFrequency.mjs ← Mide la FRECUENCIA real del veto y del grupo 2 de umbrales (funding severity, LSR contrarian) sobre 90 días de Coinalyze — lo que no se puede medir solo con klines. Resultado vigente: el veto dispara el **8,2 %** del tiempo (11 episodios/90d), así que NO se aflojó de más al recalibrar; ⚠️ **la ventana de 90 días RUEDA, así que esta cifra caduca**: la misma ejecución el 2026-08-03 da **6,6 % y 8 episodios** en SOL (y 5,6 % en BTC, 3,0 % en ETH). No es una discrepancia a resolver — son dos ventanas distintas del mismo estimador, y la conclusión ("excepción, no norma") aguanta en las dos. Citar siempre la fecha junto al porcentaje; el LSR contrarian con corte 60/40 daba `contrarian_bear` el 95,7 % y se corrigió a terciles (y en v9_0 acabó saliendo de la rúbrica). **§4 añadida el 2026-08-03 (C3): ¿precede el veto a la dirección que PROHÍBE?** Futuro de 24h con las funciones reales (`computeFirstPassage` + `classifyPathOutcome`, parámetros de `opportunityParamsFor(24)` — sin umbrales nuevos), contra el COMPLEMENTO y con anclajes disjuntos + Wilson desde el principio. **La dirección prohibida gana MÁS que el complemento en las 3 monedas y en las 2 direcciones de veto** (SOL `veto_short` +12,6 pt · BTC `veto_long` +15,7 · ETH `veto_long` +14,8), **pero los IC solapan en las tres** (n_ef 8/7/2) y también solapa el agregado optimista → **«sugerente, no establecido», NO tocar el veto**. ⚠️ **Cada moneda dispara UNA SOLA dirección del veto en 90 días** (SOL solo short, BTC/ETH solo long): la simetría del código está demostrada (fix H3), pero medir un veto con una sola moneda es imposible por construcción
  ...
  (scripts) auditPathWinRate.mjs ← **Valida `classifyPathOutcome`, el win-rate que leerá el checkpoint (2026-08-01)**. ⚠️ El ancla NO es el 50 % que se le supone: `classifyOutcome` compara precio inicial contra precio al horizonte y por eso invertir la dirección es un cambio de signo, pero ÉSTE mira el camino con barreras ASIMÉTRICAS (2×ATR a favor, 1×ATR en contra) y con dirección al azar tiene que salir muy por debajo de 50. Tres anclas fijadas antes de ejecutar: **A1 identidad exacta** — se demuestra que a lo sumo UNA dirección puede ser limpia (si `t_up(2) < t_down(1)` entonces `t_up(1) ≤ t_up(2) < t_down(1) ≤ t_down(2)`, que contradice la condición simétrica), luego `wins(Comprar)+wins(Vender)` DEBE igualar el `offered` de `classifyOpportunity`: **cuadra exacto en las 6 celdas**; **A2 ruina del jugador** — 1/(1+2)=33,3 % como techo (el horizonte finito sesga hacia la barrera cercana): medido **26,5 %** [24,7-28,3] a 24h y **19,6 %** con el par 4×/1× de 7d, contra el 20 % teórico; **A4 contra-periodo** (−270 d) 29,4 %/22,4 %, misma banda. **Hallazgo colateral: la asimetría Comprar−Vender CAMBIA DE SIGNO con la deriva** (periodo reciente −1,03 % → gana `Vender` por 5,9 pt; contra-periodo +0,64 % → gana `Comprar` por 7,4 pt; el signo coincide en las 4 celdas). No contradice el `r=−0,45` del 01-08 —aquello medía qué dirección OFRECE el mercado, no el win-rate— pero para esta métrica la deriva sí explica el grueso
  (scripts) auditBarrierTies.mjs ← **Cuánto decide en silencio el convenio "adverso primero" (2026-08-01)**. El orden intra-vela de 1h no es resoluble, así que `evaluateSetupBarrier` mira el stop ANTES que el TP1 y `classifyPathOutcome` usa `<=` a favor del stop. **En el recorrido es inocuo**: un empate exigiría que una vela de 1h cubriera 3×ATR → **0,0 %**, y el contador se demuestra VIVO (3,6 % con barreras a 0,5×/0,5×, decayendo monótonamente) para que un 0 por rareza no se confunda con un 0 por bug (fallo T1). **En el barrier del setup NO es inocuo**, porque sus niveles están mucho más juntos: stop↔TP1 en la misma vela solo el 0,18 % (mueve 0,0054R), pero **entrada↔stop el 4,83 %** — el código llena y comprueba el stop en la misma vela, y resolverlo al revés mueve la expectativa **+0,0527R, más que el IC completo de la línea base (±0,04R)**. No es un bug (el convenio es conservador y declarado) y **se CANCELA en la comparación contra la línea base** mientras ambas usen el mismo convenio, pero un `expectancy_r` ABSOLUTO no se puede leer sin esta cifra al lado. El contrafactual se mide con el evaluador REAL sustituyendo la vela de fill por una de rango cero en la entrada — no se reimplementa el barrier
  (scripts) auditVolatilityState.mjs ← **Desmonta el "24-42 %" de `volatility_state` (2026-08-01)** — ver la entrada de v8_0 · B4. Reparto real con anclas SIN SOLAPE: 35,9/31,3/32,8 ±3,4, dentro del 33,3 % que exige la construcción. Incluye el control de **bloqueo de fase** (paso de 168 velas en 1h = 7 días exactos → 85,6 % `squeeze` en BTC, contra 28,2 % sobre los mismos bloques con la fase derivando), la **réplica del muestreo real de producción** (08/20 UTC, limpio) y un **nulo Monte Carlo con caminos i.i.d. independientes** que aísla la geometría del estimador de las propiedades del mercado — es lo que demuestra que la desviación del 1W y la forma de U de los deciles son geometría y no datos
  (scripts) auditComputeTrend.mjs ← **`computeTrend` es exactamente simétrico, y `neutral` significa otra cosa de la que parece (2026-08-01)**. Ancla principal: **REFLEXIÓN** — se refleja el camino del precio y el agresor (`taker_buy' = volume − taker_buy`), bajo lo cual RSI'=100−RSI, MACD'=−MACD, DI+↔DI−, StochRSI'=100−StochRSI y buy_pressure'=100−buy_pressure, así que la etiqueta DEBE salir la contraria: **100,00 % en 3.609 ventanas**, o sea que el `? 1 : -1` de ADX/SuperTrend no tiene el caso del medio sin cubrir (el fallo de `fear_greed.trend_1d`). A3 pasa: la deriva YA REALIZADA de la ventana crece monótonamente (−9,29 → −8,20 → +0,25 → +3,18 → +5,17 %). ⚠️ **Hallazgo: `neutral` sale solo el 4,4 % y no quiere decir "sin sesgo"** — la estructura pesa 0,5 y sus dos componentes son binarios SIN banda muerta (el dead-band H5 solo llegó a ejecución), así que con acuerdo estructural (**92,9 %** de las ventanas) `neutral` exige que ejecución y volumen se opongan casi por completo y sale el **1,04 %**; con la estructura dividida sale el **47,5 %**. En la práctica `trend` es un campo de 4 valores y `neutral` ≈ "ADX y SuperTrend se contradicen"
  (scripts) auditVolumeGuardSymmetry.mjs ← **Cierra el pendiente de la asimetría del score de volumen en BTC (2026-08-01)**. Quedaba abierto un **11,8 %/34,2 %** en una muestra frente a **20,1 %/29,2 %** en otra, sin decidir si era bug. Se separa en dos preguntas: **(1) ¿lo mete el código?** NO — reflexión al **100,00 % en 50.552 ventanas** (y 46.918 más en el contra-periodo), luego `expectedVolumeScore` es incapaz de sesgar el signo. **(2) ¿difieren las dos muestras?** NO — con anclas SIN SOLAPE el reparto de BTC es **+17,3 %±7,3 / −34,6 %±9,1**, y ambos pares de cifras caen dentro de esos IC: era ruido de estimación leído como cambio, tercera aparición del mismo fallo de denominador (tras el 0,225R de la expectativa y el 24-42 % de `volatility_state`). ⚠️ **Nota de método: la reflexión ARITMÉTICA (`2p₀−p`) dio 2 falsos positivos** porque invierte las diferencias absolutas pero NO los cambios porcentuales, y `calculateCVD` decide la divergencia con un umbral del 0,1 % sobre el cambio porcentual; con reflexión **GEOMÉTRICA** (`p₀²/p`, que niega los retornos logarítmicos) el 100 % es exacto — el "fallo" era del test, no del código. **Lo que SÍ queda en pie:** el score negativo dobla al positivo en las 3 monedas y en los 2 periodos (BTC −30,2/+16,2 reciente, −26,3/+10,1 hace 9 meses). Como la simetría del código está demostrada, es propiedad del DATO (el CVD cae con convicción más a menudo de lo que sube), y tiene consecuencia direccional: la guardia C2 bloqueará tesis de volumen alcistas más a menudo que bajistas. Medir en el checkpoint, no tocar
  (scripts) auditAtrLag.mjs ← **El rezago del ATR existe pero apunta al revés de lo que se le atribuía (2026-08-01)**. El ATR de Wilder es un EMA con α=1/period y memoria ~13 periodos, así que **por definición** llega tarde; medido por PENDIENTE del ATR (n=8.919, 400 d × 3 monedas), el TR realizado de las 6 velas siguientes ÷ ATR da **0,921 comprimiendo · 0,978 estable · 1,022 expandiendo** — monótono, del signo predicho, replicado en las 3 monedas, ≈8 %. **Pero los síntomas que motivaron medirlo se describían por NIVEL de volatilidad, y por nivel manda la reversión a la media, que empuja al contrario y más fuerte: ratio 1,179 en el 4 % más tranquilo** (el régimen del periodo actual) frente a 0,857 en el 10 % más agitado. **En calma el ATR se queda CORTO un 18 %, no alto** → la banda del eje OI×precio no está inflada sino estrecha, y el lift negativo del shadow trade NO está explicado por el estimador (el sesgo iría al alza). En unidades de decisión el rezago mueve la banda un 8,6 % en compresión y un 2,3 % de media. **Lección: tres "síntomas independientes" que comparten una premisa sin medir son UNA hipótesis, no tres evidencias.** Alternativas medidas de paso: ATR7 aplana el sesgo por pendiente (1,017/0,982/0,945) pero lo desplaza; Parkinson es casi neutral al régimen pero sesgado en nivel ×1,45 (ignora los huecos) → exigiría una constante de reescalado, o sea otra constante que medir
  (scripts) auditAtrRounding.mjs ← **El redondeo del ATR y los topes absolutos: medidos, ninguno exige tocar nada (2026-08-01)**. `calculateATR` redondea a 2 decimales en unidades de PRECIO, así que el error RELATIVO depende del activo: **0,53 % en SOL/1h · 0,02 % ETH · 0,0003 % BTC**. Efecto en la banda del eje OI×precio **≤0,008 pp** y **solo el 0,139 % de las celdas cambian** → **no se toca**, porque quitar el redondeo movería todos los umbrales normalizados y exigiría punto cero a cambio de nada. Riesgo latente de escala anotado (un activo de céntimos rompería). Y los topes ABSOLUTOS de `dynamicNearLevelPct`, que son el único punto donde producción compara ATR% contra una constante entre activos: en el **TF primario 4h el suelo no muerde nunca y el techo el 5,9 %** (1h: suelo 8,9 % global pero **47,9 % en el decil más tranquilo**; 1W: techo 46,8 %) → sin recorte relevante mientras el primario sea 4h. ⚠️ La 1ª versión daba **100 % de recorte en los 4 TFs**: comparaba contra la salida ya redondeada de la función en vez de contra los límites — un 100 % exacto repetido en cuatro filas es firma de artefacto
  (scripts) auditOpportunityRegimeCurve.mjs ← **Tumbó el cambio que venía a justificar (2026-08-01)**. Se iba a convertir `OPPORTUNITY_BASE_RATE` en tabla por régimen tras medir 45,6 % en el cuartil tranquilo frente al 34,8 % publicado. Este script preguntó primero **con qué se INDEXA la tabla**, y ahí se cayó: los cuartiles de ATR% **absoluto** son casi una clasificación por MONEDA (en 7d, Q1 = 685 anclas de BTC contra 64 de SOL). Con clave relativa (percentil del ATR% en su propia ventana) **y anclajes de ventanas DISJUNTAS** (anclas cada 4h con horizonte 24h comparten 5/6 del futuro): **41,7 / 33,5 / 33,8 / 29,1 / 31,2 %, global 35,2 % [31,9-38,6] — los cinco IC solapados y todos conteniendo el global.** El 34,8 % está bien y **la constante no se toca**. Lo que sí salió de aquí es `atr.pct_percentile`, la covariable sin la cual no se puede condicionar ninguna tasa base a posteriori
  (scripts) auditOrderlyDecline.mjs ← **La caída ordenada NO es vendible, y la asimetría de la rúbrica es correcta (2026-08-01)**. Toda la fila inferior del cuadro OI×precio vale 0 (`new_money_short`, `deleveraging`), así que el score de la cima es mudo justo en la caída más común y su mudez bloquea `Vender`. Medida la pregunta **CONDICIONAL** que faltaba —no "¿predice `OI↑px↓` solo?" (ya respondida: no) sino "¿predice cuando estructura y volumen acompañan?"— sobre 90 d × 3 monedas (n=1.584), con la base del grupo «precio pasado ↓» como referencia obligatoria: **+2,1 pt (n=97, IC cruzando la base)**, bajo el listón de 5 pt, sin replicar (SOL +25,2 con n=20 · BTC −8,9 · ETH +2,1) y con estructura+volumen SIN OI dando el mismo +2,1. `deleveraging` incluso continúa a la baja MENOS que la base (−9,6). **La limitación se reclasifica de deuda a propiedad del mercado.** ⚠️ **El control simétrico alcista es el hallazgo:** `new_money_long` +6,8 y **condicionado por estructura+volumen +12,4** (n=53), frente a **+0,4 de estructura+volumen SIN el OI (n=90)** — en el lado largo la información está en el OI y no en los otros bloques; en el corto no está en ninguno. Replica en signo en las 3 monedas con n=27/11/15 → **sugerente, no establecido**. Si se confirma, la vía es **reforzar la puerta de COMPRA, no abrir la de venta**. Proxies deterministas (`computeTrend` + `expectedVolumeScore`), conservadores frente al LLM
  (scripts) auditBaseRateConditioning.mjs ← **Las tasas base no estaban condicionadas al régimen (2026-08-01)**. Producción muestrea a hora fija (08/20 UTC) y lleva el periodo en el cuartil más tranquilo, pero `OPPORTUNITY_BASE_RATE` y `TRIGGER_BASE_RATE` se midieron sin condicionar. **La HORA no importa** (oportunidad 33,1 % vs 33,8 % a 08/20 UTC; `no_signal` 69,4 % vs 68,9 %) → hipótesis de estacionalidad horaria descartada. **El RÉGIMEN sí, y rompe una invariancia prometida por construcción:** la tasa de oportunidad va **45,6 / 31,6 / 29,5 / 25,8 %** de Q1 a Q4 de ATR% cuando, al estar ATR-normalizada, debería ser plana — y el signo coincide con la predicción firmada de `auditAtrLag` (ATR corto en calma → listón de 2×ATR bajo → más oportunidades), segunda confirmación por otra vía. **`offered_pct` se juzgaba contra 34,8 % cuando en este régimen toca 45,6 %.** La curva de disparo en cambio **sí calibra donde vive producción** (+2,3 en Q1), lo que DESMIENTE la explicación que se le había dado al lift −55 del shadow trade. Y cierra `no_signal`: **69,4 % histórico**, estable por hora y régimen → es la celda mayoritaria de siempre. ⚠️ **LA ACCIÓN PROPUESTA AQUÍ FUE RETRACTADA EL MISMO DÍA — NO implementar.** Se propuso `OPPORTUNITY_BASE_RATE` como tabla por cuartil de ATR%; al ir a escribirla, `auditOpportunityRegimeCurve.mjs` demostró que **el efecto no sobrevive** a las dos correcciones (clave RELATIVA en vez de ATR% absoluto, que era casi una clasificación por moneda; y anclajes DISJUNTOS en vez de solapados): los cinco IC se solapan y todos contienen el global. **El 34,8 % está bien y la constante NO se toca** — y por tanto **el «45,6 %» de esta entrada tampoco se usa para corregir ningún `lift` a mano**: es la cifra confundida. Lo único que queda vivo de aquí es (1) la hora no importa, (2) `no_signal` 69,4 %, (3) el lift −55 del shadow trade sigue sin explicación, y (4) `atr.pct_percentile`, la covariable para estratificar A POSTERIORI en el checkpoint. Ver la entrada de `auditOpportunityRegimeCurve.mjs`
  (scripts) auditTargetReachability.mjs ← **El corte que reemplaza al de R:R, este sí medido (2026-08-01)**. No juzga CALIDAD (eso se midió y es plano en R:R) sino **coherencia de la declaración**: si el TP1 está a una distancia que el mercado no recorre en las velas que el propio análisis declara, el resultado lo decide la CADUCIDAD y no el objetivo → declaración inerte, mismo tipo de defecto que `conditional_trigger_vague`. **Eje:** `d = distancia% / (ATR% × √velas)` (el de `normalizedTriggerDistance`); con la distancia en múltiplos de ATR el ATR se cancela y queda `d = k/√V`. **El colapso se comprobó**: celdas con la misma `d` pero `k`/`V` distintas coinciden dentro de **0,3-2,6 pt** — una curva de UNA variable, igual que `TRIGGER_BASE_RATE`. **⚠️ El anclaje propuesto se cayó:** se intentó heredar el nivel de `OPPORTUNITY_BY_HORIZON` (si 2×ATR/6 velas y 4×ATR/42 dieran la misma alcanzabilidad, ése sería el corte sin inventar nada) y dan **22,2 % vs 32,0 %** con IC sin rozarse — aquel par se calibró sobre la oportunidad LIMPIA, que no es esta magnitud. Anotado para que nadie lo reintente. **Lo que sostiene el nivel es mejor que un anclaje: cualquier corte entre el 3 % y el 10 % da la MISMA partición** de las geometrías reales → el nivel no está sobre una pendiente donde un pelo cambia la respuesta (lo contrario del fallo T2). Se toma el **5 %** (`TARGET_UNREACHABLE_PCT`), centro de la banda y coherente con el listón de rama muerta ya aplicado cinco veces; y no es rama muerta él mismo: marca 2 de 7 condicionales reales. `ATR_WINDOW=180|19` para comprobar que la curva no depende de la ventana del ATR (≤1,6 pt)
  (scripts) auditConditionalRR.mjs ← **El umbral de R:R no medía nada y se retiró (2026-08-01)**. `conditional_low_rr` avisaba por debajo de **R:R < 1**, corte nunca medido y sin ningún test — y la 8ª medición trajo un condicional con **R:R 1,00 exacto**, justo en el borde. La pregunta no fue dónde mover el corte sino **si el corte separa algo**, con la predicción escrita antes de ejecutar: sobre un paseo casi sin deriva `P(TP primero) ≈ riesgo/(riesgo+recompensa)`, luego el acierto debe caer exactamente lo que sube el premio y la expectativa salir plana. **Dos barridos, porque un cociente se mueve por arriba o por abajo** — A · stop fijo 1×ATR con objetivo variable · B · objetivo fijo 2×ATR con stop variable: si coinciden manda el cociente, si no manda la DISTANCIA y "R:R" sería la variable equivocada. 7 valores × 3 monedas, n≈1.945-3.204 réplicas/celda, con `evaluateShadowTrade` + `summarizeShadowTrades` REALES (mismo arnés que `auditShadowBaseline`). **B plano (−0,006 a +0,008R, IC conteniendo 0); A con gradiente (+0,204R a R:R 0,5 → +0,002R a 2,0), o sea al revés de lo que el aviso presupone.** ⚠️ **El control salva la conclusión:** con la entrada EN el precio (`ENTRY_K=0`) el gradiente se aplana a −0,004R y el acierto **calca el equilibrio** (67,1 vs 66,7 · 57,5 vs 57,1 · 50,0 vs 50,0 · 43,5 vs 44,4) → el `+0,204R` era **selección por el llenado** (la entrada está 0,75×ATR en contra y solo se llena tras un movimiento adverso, capturando reversión a la media), no una propiedad del R:R. **Un R:R bajo no es peor geometría, es otra apuesta**; lo único que cambia con él es `1/(1+R:R)`, que ya viaja en `breakeven_win_rate_pct`. Hallazgo colateral SIN escribir: el objetivo caduca sin tocarse el **4-7 %** a 0,5×ATR y el **43-50 %** a 3×ATR dentro de 24h → *"objetivo inalcanzable en la vigencia declarada"* es el candidato medible que reemplazaría al corte retirado, pero exige su propia medición
  ...
  (scripts) auditSrTolerance.mjs ← **U2 (2026-08-03) · el % absoluto de agrupamiento S/R NO es inocuo**. `SR_TOLERANCE_PCT = 0.005` decide qué pivotes forman un mismo nivel, y es ABSOLUTO — mientras que `dynamicNearLevelPct`, que juzga la CERCANÍA a esos niveles, se normalizó por ATR en T5: los niveles se construyen con un criterio y se juzgan con otro. Tres predicciones firmadas antes de ejecutar, las tres cumplidas: la tolerancia en unidades de ATR difiere **×1,55-1,80** entre monedas, y la fracción que pasa `touches >= 3` —**el filtro del VETO**— dispersa **11,6-12,3 pt**; con tolerancia normalizada cae a **1,4-2,5 pt**. ⚠️ **El hallazgo mayor es entre TIMEFRAMES**: 1h 42,9-55,2 % · 4h 18,0-29,5 % · **1D 2,9-5,5 %** — en 1D casi nada llega a "nivel fuerte". Y el TF primario es un parámetro de petición, así que un análisis en 1h tiene el DOBLE de niveles fuertes que uno en 4h. Control: el 18-29,5 % de 4h replica el 22,1 % de T4. **No se toca ahora** (cambia la frecuencia del veto, que está medida): entra en el punto cero como F2, con ancla propuesta `k ≈ 0,35` para dejar 4h sin cambios
  (scripts) auditDirectionalBias.mjs ← **FASE 0 (2026-08-03) · NO-GO en 3 de 3 monedas**. ¿Bate un `bias` determinista (derivados+volumen+ejecución) al AZAR y a la DERIVA? Cuatro brazos sobre las MISMAS anclas (azar por hash del instante, bias, deriva, oráculo), un solo ensayo por ancla, IC sobre anclajes disjuntos. Resultado: el bias queda **por debajo del azar en las tres** y no supera a la deriva en ninguna — perdiendo además **in-sample** (la rúbrica se calibró sobre esos días). ⚠️ **El techo del ORÁCULO sale 39,5-55,0 % contra un azar del 16-27 %**: la señal direccional EXISTE, lo que no existe es una feature que la capture. ⚠️ Bug cazado por su propio control: el brazo de azar alternaba por paridad del índice y **se aliasaba con el espaciado de la cadena disjunta** (1 de cada 6 anclas) dando dirección CONSTANTE → 8,5 % contra el 24,1 % de la mezcla exacta. Sin ese control el informe habría publicado un azar del 8,5 % y el bias habría "ganado" por 8 puntos
  (scripts) auditCleanMoveDirection.mjs ← **M9 (2026-08-03) · NO-GO**. Condicionado a que el mercado SÍ ofreció movimiento limpio, ¿alguna de **7 features PRE-REGISTRADAS** predice su signo? Nulo declarado = la **clase mayoritaria** (adivinar la dirección dominante del periodo es gratis), no el 50 %. En el periodo principal **6 de 7 replicaban en signo** y parecía una firma de reversión a la media coherente con M8 — **el contra-periodo lo tumbó**: `st_4h` y `smc_bos` cambian de signo. **Es el caso de estudio del pre-registro**: sin la condición (c) escrita ANTES, se habría publicado un hallazgo coherente, replicado en 3 monedas… y falso
  (scripts) auditEntryGeometry.mjs ← **M8 (2026-08-03)**. La geometría de ENTRADA que el producto enseña. Se reformuló al escribirla: la pregunta literal ya estaba contestada por tres mediciones. Hallazgo previo que lo justifica: `auditConditionalRR` midió entradas a RETROCESO, pero los setups reales son de ROTURA — **se había medido la geometría opuesta**. Resultado con la geometría real (barreras 1,7×/3,4×ATR): rotura −0,02/−0,09 · mercado +0,02 · **retroceso +0,07/+0,16**, monótono y replicado pero **sin IC que separe de 0**. Firme: **el sistema emite el extremo PEOR**. Resolvió además una discrepancia de 0,3R con `auditShadowBaseline` que parecía un bug: medían geometrías distintas (barreras estrechas ⇒ casi todo RESUELVE y la asimetría 2:1 muerde; anchas ⇒ la mayoría CADUCA a ≈0R). **Consecuencia: la línea base de la expectativa es una CURVA** indexada por anchura÷√vigencia, no un número — tercera de la misma familia tras `TRIGGER_BASE_RATE` y `TARGET_REACHABILITY`
  (scripts) auditFillRule.mjs ← **M7 (2026-08-03)**. ¿Cuánto cuesta que el evaluador llene al TOCAR y el gatillo pida CERRAR? Tres ramas con el evaluador REAL (sólo cambia lo que se le pasa): A toque 56,9 % de llenado · B cierre 40,4 % · **C confirma+límite 30,6 %** ← lo que el setup describe. ⚠️ **Autocorrección**: con sólo A y B se concluyó que el setup era "internamente incoherente"; al releer un caso real (gatillo *cierre 4h < 72,09*, entrada **71,90**, un nivel más abajo) se ve que describe *break-and-retest* y es coherente — la rama C no estaba medida. **Lo que se retracta:** que el R:R del panel engañe (bajo C el declarado ES el realizable). **Lo que queda:** el registro **infla la ACTIVIDAD casi al doble**. P3 resultó ser **un campo de esquema que faltaba** (`trigger_price`), no un cambio de regla
  (scripts) backfillSampleReason.mjs ← Rellena `analyses.sample_reason` en filas anteriores a la columna. ⚠️ **Lee el LOG, no la hora**: `collect.log` registra el motivo de cada ejecución, así que el dato existía y sólo estaba en otro sitio — **deducir lo que ya está escrito es inventar donde se puede leer**. La heurística horaria queda de respaldo para filas sin línea de log, y ésas van marcadas `:inferred`. 13/13 con motivo registrado
  (scripts) backfillLiquidationsHourly.mjs ← **Siembra `liquidations_1h`** con los ~90 días que Coinalyze todavía sirve (3 monedas, `--dry-run`, `--force`). No reimplementa nada: escribe con `addLiquidationsHourlyEntries`, la misma función que el camino en vivo. Informa de si el resultado satisface `cascade_min_points: 620` — un backfill que no llegue ahí no habilita lo que vino a habilitar. **Se ejecuta cuanto antes y una sola vez**: la ventana de la API rueda, así que cada día sin sembrar es un día perdido para siempre
...
src/utils/
  ...
  stats.js                   ← IC de Wilson + `summarizeShadowTrades` (**2026-08-01**: `trigger_rate_pct` = ¿llegan a darse las condiciones que el sistema nombra?, y `win_rate` sobre `tp1+stop` con el MISMO gate `MIN_DIRECTIONAL_SAMPLE`. ⚠️ **CENSURA corregida en el denominador**: un condicional que dispara puede resolverse en la 1ª hora mientras uno que no dispara no es terminal hasta agotar su vigencia, así que contar por "tener resultado" sesgaría `trigger_rate` AL ALZA — solo entran los que tienen la **ventana vencida** (`pending_n` aparte). La vigencia se recalcula con `setupExpiryMs`, no se persiste: es función exacta de datos ya guardados. ⚠️ **A `trigger_rate_pct` le falta su tasa base** — sin ella es el mismo número suelto que era `offered_pct` antes de `OPPORTUNITY_BASE_RATE`; depende de cada geometría, así que exige script propio) + **Fase 5**: `classifyOpportunity` (¿ofreció el mercado un movimiento operable? = `targetK`×ATR antes de `adverseK`×ATR en contra, SIN dirección propia. ⚠️ **CENSURA corregida el 2026-08-01 — cuarto denominador mal contado**: marcaba `evaluable: true` con solo existir la rejilla de cruces, sin exigir que el horizonte hubiera VENCIDO, y con `horizonH = null` (el modo del bloque de 7d) `crossedAt` no pone tope y usa la ventana parcial ya escrita por el job → el endpoint publicaba `offered_pct 0,0` y **`lift −36`** sobre 7 filas de las que ninguna llegaba a 66 h de las 168 h del horizonte: **cero observaciones maduras presentadas como abstención brillante**, y justo la cifra que el checkpoint mira. **La asimetría es la regla:** un `offered` es TERMINAL en cuanto se observa el cruce —más mercado no deshace un recorrido ya ofrecido— pero un `offered:false` solo significa *"todavía no"*. **`blocked_by_adverse` también es terminal** aunque la ventana siga abierta: cruzar `targetK` implica haber cruzado antes `adverseK` en ese sentido (paso monótono, `targetK > adverseK`), luego cualquier objetivo futuro llega con su adverso por delante. Sin cruce y con ventana abierta → `pending`, fuera del denominador y visible en `pending_n` (mismo patrón que `summarizeShadowTrades`); el vencimiento del bloque de 7d se ancla a `SHADOW_MAX_WINDOW_MS`, ya dueño de "la ventana de datos del job", en vez de escribir un segundo *7 días*; sin fecha utilizable → `pending` también, porque un dato ausente no es una abstención acertada. Efecto real: 7d de `0,0 %/−36` a **`null` con `pending_n: 7`**, y **24h intacto** (42,9 %) — sus negativos ya eran maduros, que es la prueba de que el gate discrimina. **Un solo dueño de la censura, `horizonMatured`**, compartido con `classifyPathOutcome`: si divergieran, el coste de oportunidad y el win-rate estarían midiendo sobre muestras distintas sin decirlo. ⚠️ **`opts.now = null` DESACTIVA la censura y hay que pasarlo explícitamente en los scripts de auditoría**, que replican historia cerrada con filas SIN `timestamp` y llevan su propio gate de cobertura sobre las klines. No es un detalle: al introducir el gate, `auditOpportunityRegimeCurve` y `auditBaseRateConditioning` —que **filtran por `evaluable`**— pasaron a reportar ~100 % de oportunidad **en silencio**. Un cambio de semántica en una función pura se rastrea hasta TODOS sus llamadores, y los scripts son llamadores de primera clase sin tests que los protejan), `classifyPathOutcome` (win-rate path-aware: mira el camino, no el destino. ⚠️ **Misma censura, y aquí SESGA EL SIGNO — corregido el 2026-08-01**: `win`/`loss` son terminales en cuanto se tocan, pero `flat` es "aún no" y está FUERA del denominador (`directional_n = win + loss`); como el stop está a `adverseK` y el objetivo a `targetK` con `targetK > adverseK`, **la pérdida aflora antes que la ganancia** y el win-rate de una muestra en curso sale sesgado A LA BAJA. Un `flat` con la ventana abierta devuelve ahora `'pending'` y no entra en nada), `maxExcursionAtr`, `convictionBucket`, y los agregadores `summarizeOpportunity`/`summarizePathWinRate`/`summarizeConviction`. `OPPORTUNITY_DEFAULTS` (2×/1×ATR) son CONVENCIONES sin calibrar — viajan en la respuesta (`thresholds`) para que la cifra nunca se lea sin su regla. `MIN_DIRECTIONAL_SAMPLE=20` vive aquí para que el win-rate clásico y el path-aware compartan gate. **`TRIGGER_BASE_RATE` (2026-08-01)** — la referencia que le faltaba a `trigger_rate_pct`, que era un número suelto igual que `offered_pct` antes de su tasa base. Se creía que había que medirla *por condicional* porque depende de la geometría; medido (`scripts/auditTriggerBaseRate.mjs`), al normalizar la distancia por **`ATR% × √velas`** la tasa **COLAPSA**: 24h y 48h coinciden dentro de 0,4-6,4 pt y las tres monedas dentro de 0,7-3,4 pt → es una **curva de UNA variable**, enviada como constante con `measured_at` (mismo patrón que `OPPORTUNITY_BASE_RATE`). `normalizedTriggerDistance` convierte la vigencia al TF del ATR (un condicional ejecutado en 1h con ATR de 4h daría distancia inflada ×2) y `triggerBaseRateFor` interpola sin extrapolar. ⚠️ El eje usa `atr_pct_at_analysis` (19 velas), NO el `components.atr_pct` de decisión (180): se eligió porque está persistido retroactivamente en TODAS las filas — para una tasa base da igual cuál sea mientras tabla y consumidor usen el mismo. **`expectancy_r` + `breakeven_win_rate_pct`**: el win-rate solo no es interpretable (con R:R 1,77 el equilibrio está en 36,2 %, así que un 40 % gana y un 33 % pierde, y ni con n=40 el IC excluye ese punto); `rr_median`/`breakeven` describen la GEOMETRÍA y están disponibles sin resultados, `expectancy_r` es el RESULTADO y comparte gate con el win-rate. El modal pintaba el acierto contra un **50 % hardcodeado** — corregido para pintarlo contra el equilibrio real. **`expectancyR` (2026-08-01, opción A):** `expectancy_r` es un OBJETO `{point, ci_low, ci_high, n, inconclusive}`, no un número — el punto no se puede leer sin su intervalo, misma disciplina que `fill_rule` con `trigger_rate`. Usa **t de Student** (a n=8 el margen es 21 % mayor que con z, y n=8 es el tamaño del primer checkpoint). **SIN gate de muestra, a diferencia del win-rate**, y es deliberado: `MIN_DIRECTIONAL_SAMPLE=20` está calibrado para una PROPORCIÓN, y para la media de un múltiplo de R con cola es demasiado pequeño — a n=20 el IC sigue siendo ±0,57R y a n=60 ±0,33R, así que un gate protege justo cuando menos falta (n=8, donde el número es obviamente inútil) y falla justo cuando más falta (n=60, donde ya PARECE serio y sigue sin afirmar el signo). `inconclusive` es proporcional a la incertidumbre real en todo n. Medido con R:R 1,65: hacen falta ~64 resueltos para una ventaja fuerte (+0,32R) y **~181 para una moderada** (+0,19R) → a 0,4-0,7 resueltos/día es una pregunta de MESES, no de checkpoint. En el modal, si el intervalo cruza cero el TITULAR es el rango y el punto baja al tooltip. ⚠️ **DENOMINADOR CORREGIDO el 2026-08-01**: la expectativa contaba solo `tp1`+`stop`, y ese subconjunto está enriquecido en stops **por la GEOMETRÍA**, no por el mercado — el stop suele estar más cerca que el objetivo, así que dentro de una vigencia corta lo alcanza mucho más a menudo y lo que no llega a ninguno CADUCA. Medido con 3.733 réplicas de las 7 geometrías reales: **1.172 caducados frente a 1.064 resueltos**, o sea que se tiraba el 52 % de los trades disparados siempre por el mismo lado. Ahora entra todo lo llenado y el caducado aporta su R real vía `analysis_outcome.cond_exit_price` (se persiste el PRECIO, no el R: el R sale de una sola fórmula válida para largo y corto, `(salida − entrada)/(entrada − stop)`, que reproduce +R:R en `tp1` y −1 en `stop`). **El sesgo era de 0,225R**: la línea base pasó de −0,221R [−0,294, −0,148] *concluyentemente negativa* a **+0,004R [−0,036, +0,044]**, que es lo que debe dar una geometría aplicada al azar. `scripts/auditShadowBaseline.mjs` mide esa línea base — sin ella un `−0,1R` real no se sabe si es bueno o malo. **`TARGET_REACHABILITY` + `normalizedTargetDistance` + `targetReachabilityFor` + `TARGET_UNREACHABLE_PCT` (2026-08-01)** — la curva que sustituye al corte de R:R retirado, y **el dueño único de "cuánta distancia cabe en N velas"**. `normalizedTargetDistance` delega en `normalizedTriggerDistance` (misma conversión de TF: un condicional ejecutado en 1h con ATR de 4h daría distancia inflada ×2), así que gatillo y objetivo comparten eje. Con la distancia en múltiplos de ATR el ATR **se cancela** y queda `d = k/√V` — **colapso comprobado dentro de 0,3-2,6 pt** entre celdas con la misma `d` y distinto `(k,V)`, el mismo grado que dio `TRIGGER_BASE_RATE`. Medida con el ATR de **180** velas, el de DECISIÓN, porque es el que usa el consumidor (la versión con 19 difiere ≤1,6 pt: la elección se cancela, pero se alinean igual). El corte (5 %) **no se pudo heredar** de `OPPORTUNITY_BY_HORIZON` —sus dos puntos calibrados dan 22,2 % vs 32,0 %, IC sin rozarse, porque aquel par se calibró sobre la oportunidad LIMPIA— y se sostiene en algo mejor: **cualquier nivel entre el 3 % y el 10 % da la misma partición** de las geometrías reales. Consumida por `computeTargetReachability` (analysisController) → `applyDecisionGates` → `opts.targetReachability` del validador; **nunca importada por `analysisValidator`**, que se declara sin dependencias. ⚠️ **`offered_pct`/`trigger_rate_pct` sin IC — corregido el 2026-08-09.** Eran las dos únicas proporciones de toda la familia (junto a `win_rate`/`expectancy_r`, que sí lo llevan) sin intervalo de Wilson: un lift negativo se leía como "el sistema elige peor que el azar" cuando podía ser simple ruido de muestra. Verificado con los 56 primeros análisis: en las 4 celdas medidas (oportunidad 24h, trigger global/`long`/`short`) la tasa base cae DENTRO del IC de Wilson en las 4 — ninguna es distinguible de azar todavía, ni siquiera `short` (lift −22,4, el caso más extremo, con la base justo al borde del IC). `summarizeOpportunity` añade `offered_pct_ci_low/high` + `lift_significant`; el `tally()` interno de `summarizeShadowTrades` añade `trigger_rate_ci_low/high` + `trigger_lift_significant` (booleano: `true` solo si la base cae FUERA del IC). El modal (`history.js`) ya no colorea ni titula el punto crudo cuando `*_significant` es `false` — mismo tratamiento que `expectancy_r.inconclusive`: el titular pasa a ser el rango del IC y el punto baja al tooltip. Sin decisión de código: es telemetría de lectura, no toca la ruta de decisión
  conditionalPlan.js         ← **[RETIRADO 2026-08-10, pivot a ayudante de riesgo — el archivo YA NO EXISTE en `src/`, sustituido por `riskGeometry.js` (geometría SIMÉTRICA largo+corto, sin dirección elegida), ver CLAUDE.md §REORIENTACIÓN]** Describe un `conditional_setup` para el PANEL (2026-08-03). Función pura que traduce el plan a cuatro cifras **con respaldo medido** y se niega a inventar el resto: `rr` + `breakeven_win_rate_pct` (aritmética pura), `trigger_prob_pct` (`TRIGGER_BASE_RATE`), `target_reachability_pct` (`TARGET_REACHABILITY` — en los setups reales sale al **5-18 %**, o sea que lo normal es que el plan caduque abierto) y `expires_at`. ⚠️ **NO expone `expectancy_r`** (su línea base es una curva sin medir, M10) **ni ninguna probabilidad de acierto direccional** (fase 0 y M9: NO-GO); hay tests que lo protegen. El `fill_rule` viaja SIEMPRE pegado a las cifras. Sin ATR o sin vigencia devuelve `null`, nunca un valor por defecto. ⚠️ **La caducidad la calcula `setupExpiryMs`, la del EVALUADOR** — replicar la aritmética permitiría que el panel prometiera un vencimiento distinto del que se aplica (B3)
  riskGeometry.js            ← **[NUEVO, 2026-08-10]** `computeRiskGeometry()` — geometría de riesgo SIMÉTRICA (largo Y corto a la vez, nunca elegidos), función pura. Reutiliza las curvas ya medidas: `OPPORTUNITY_BY_HORIZON['24h']` (1×ATR stop / 2×ATR objetivo, de `stats.js`) + `TARGET_REACHABILITY`/`targetReachabilityFor` (alcanzabilidad del objetivo) + `setupExpiryMs` (vigencia, de `outcome.js`) — ningún múltiplo nuevo, ninguna constante sin medir. Vigencia fija a 24h (6 velas en TF 4h), decisión de producto, no medida. Usa el ATR de DECISIÓN (180 velas, `technical[primary_tf].atr.pct`), SÍNCRONO — no depende de nada que el LLM declare, así que se calcula en `assembleAnalyzeContext` antes de llamar al LLM y no hace falta esperar al job de outcome para tenerla completa (a diferencia del `conditionalPlan.js` al que sustituye). Se niega a devolver `expectancy_r` (línea base sin medir, M10) y cualquier probabilidad de acierto direccional — `rr`/`breakeven_win_rate_pct` son IDÉNTICOS en `long`/`short` por construcción, prueba de que no codifican opinión sobre cuál gana
  sampleReason.js            ← **Dueño único de "de dónde vino esta observación" (2026-08-03)**. `normalizeSampleReason` valida entrada de usuario que acaba en BBDD: prefijo del vocabulario (`fixed`/`opportunistic`/`ui`/`manual`/`adhoc`/`unknown`) + detalle opcional tras `:`, minúsculas, tope 64. Lo que no encaja → `unknown` (información honesta, no un origen inventado). Convierte la convención antigua `manual_verificacion` → `manual:verificacion`. Compartido por la API y el backfill: nació duplicado y se unificó el mismo día
...
**`GET /api/analyze/payload` añade además:** `order_book` (muros + imbalance), `derivatives.liquidation_clusters` (magnetic zones inferidas + flags `magnetic_long/short_zone_active`), `onchain` (MVRV/NUPL/SOPR para BTC), `etf_flows` (BTC/ETH spot ETF flows vía SoSoValue), `macro` (DXY/SPX/Gold vía Yahoo Finance), `volatility` (btc_dvol/eth_dvol via Deribit — value, regime, change_24h_pct), `volume_history` (CVD/VWAP), `timeframe_analysis` (conflictos entre TFs), **`risk_geometry`** (geometría de riesgo SIMÉTRICA largo+corto, `utils/riskGeometry.js` — sustituye a `gating`/`expected_scores`, retirados con el pivot del 2026-08-10, ver §REORIENTACIÓN), `derivatives_score` (`basis[]`/`components` como color de lectura de derivados; el `.score` agregado y sus tres sumandos se excluyen del dataset que recibe el LLM en `buildPrompt` — ver `utils/derivativesScore.js`), resúmenes consolidados de históricos. `technical[tf]` incluye `atr` ({value, pct, period} — proxy de vol realizada, cubre a SOL sin DVOL). `technical[tf]` incluye además `volume_profile` (con `price_vs_poc`, `excursion`) y `smc` (BOS/CHoCH/FVGs con `signal_status`); `cvd.cvd_strength` (+ `cvd_strength_pctile` y `cvd_strength_cuts`, telemetría de la calibración) y `vwap.price_vs_vwap` también precalculados.
...
frontend/
  ...
  assets/
    ...
    js/
      app.js                 ← Entry point: init PixiJS, carga datos, conecta eventos, persistencia. **`restoreRecommendationPanel(rec, {serverAnswered})`**: el orden de precedencia es SERVIDOR → puente local → vacío. La copia de `localStorage` es un PUENTE para que el panel no parpadee mientras llega `/api/data`, **no una memoria paralela**: en cuanto el backend contesta (`serverAnswered:true`), un `last_analysis` nulo significa "no hay análisis" y la copia local se PURGA (`clearCoinRecommendation`). Sin esa distinción el fallback no separaba "aún no ha contestado" de "ha dicho que no hay nada", y un análisis viejo sobrevivía a un borrado de BBDD y a un redespliegue, repintado idéntico a uno recién hecho (bug detectado en el punto cero 5, 2026-07-29). runAnalysis() lee el modelo del desplegable `#model-select` (persistido en localStorage `cryptex_model`), muestra "⏳ Analizando…" en el botón y al terminar abre el modal de Historial (openHistory) con el resultado + previos. ⚠️ **`runAnalysis()` envía como `primary_tf` el TF DEL GRÁFICO** (`const { coin, tf } = getState()` → `postAnalyze(coin, tf, model)`), y el backend lo acepta contra `TIMEFRAMES` completo con default `4h` ([analysisController.js:1195](backend/src/controllers/analysisController.js#L1195)). O sea que **el TF primario del análisis es un parámetro de petición, no una constante**: poner el chart en 1h y pulsar el rayo produce un análisis con primario 1h sin tocar código. El cron de recogida lo fija (`TF="${TF:-4h}"`), así que la muestra planificada es homogénea. **Importa porque el TF primario gobierna cosas que se comparan entre análisis**: el episodio es *una vela del TF primario* (`episodes.js`), la vigencia del setup se cuenta en velas de ese TF y la banda del eje OI usa su `√n` — un análisis de otro TF no es incorrecto, es **no comparable**. Y el suelo de `dynamicNearLevelPct` (0,5 %) muerde el **47,9 % del decil más tranquilo en 1h** frente al **0,0 % en 4h** (2026-08-01, `auditAtrRounding.mjs`): por debajo de ATR%=0,33 el umbral deja de encogerse y `price_near_key_level` dispara de más justo en calma (fallo T5)
      api/
...
- `analysis_liquidation_snapshot` — hasta 10 filas por análisis (5 long + 5 short): clusters de liquidación persistidos en el momento del análisis. ⚠️ **estuvo VACÍA hasta el 2026-08-02 por un bug de nombre de clave**: `buildClusterRows` leía `top_long_clusters`/`top_short_clusters` y el servicio emite `long_clusters`/`short_clusters`, así que el `?? []` se tragaba el bloque **en silencio** (el LLM sí veía los clusters — la sección F5 consume `magnetic_long/short_zone_active`; lo que se perdía era auditarlos a posteriori). Corregido, con `distance_pct` derivado contra `price_current` igual que en `buildFvgRows`, y **backfilleado** (`scripts/backfillLiquidationClusters.mjs`, 100 filas / 10 análisis). Columna `reconstructed` (0 = escrita en vivo, 1 = reconstruida): la reconstrucción **no es bit-exacta** porque la última hora de la ventana estaba a medio formar en el análisis y está cerrada en el backfill → los bins pueden moverse un poco (`distance_pct` sí es exacto, sale del spot persistido). El algoritmo vive en `utils/liquidationClusters.js` (puro) con **un solo dueño** para que servicio y backfill no diverjan — que es exactamente cómo nació el bug.
...
- `history_series` — series históricas persistidas (`coin`, `metric`, `ts_key`, `payload` JSON, PK compuesta). Alimentada por `historyService.js` write-through para las 7 métricas (funding/oi/lsr/liq/cvd/vwap/fear_greed; fear_greed bajo coin `GLOBAL`). **No se dropea** en migraciones — sobrevive reinicios. Retención 400 días. Ver excepción a la regla de abajo. **`liquidations_1h` (2026-08-03) — 8ª métrica, serie de ARCHIVO**: liquidaciones en resolución HORARIA, sólo BBDD (no se hidrata en memoria ni entra en la ventana del LLM — 720 filas costarían tokens sin aportar lectura). **Por qué existe:** `liquidationCascade` normaliza contra la mediana de ~697 ventanas RODANTES de 24 h y se abstiene por `cascade_min_points: 620`; desde agregados diarios ese guard **no se satisface jamás** (30 puntos/mes), así que la cascada sería muda en cualquier auditoría a posteriori — y es la única vía negativa que sobrevive en una caída (D6). **Qué compra:** Coinalyze sirve 90 días y esa ventana **rueda y olvida**; la rúbrica se calibró el 2026-07-29 sobre los 90 anteriores, así que hoy no existe ninguna ventana fuera de muestra y **con la API sola no existiría nunca**. Esta serie es el único mecanismo que rompe ese muro. **Métrica NUEVA y no un cambio de `liquidations`**: dos granularidades bajo la misma clave serían dos definiciones en la misma serie (lo que `backfillHistorySeries` aborta por diseño con el CVD `heuristic` vs `taker_real`). Escritura **incremental** (sólo posterior a `MAX(ts_key)` menos 2 h de margen, porque la última vela horaria puede estar formándose al descargarla) → ~2-3 filas por poll en régimen, y se autorrellena tras una caída hasta donde alcance la ventana. ⚠️ Por eso **rellenar hacia atrás exige `--force`** en el backfill si el poller ya escribió (pasó en el despliegue: el poller metió su ventana de 30 d antes y el backfill sólo añadió 2 filas). Sembrada el 2026-08-03 con `scripts/backfillLiquidationsHourly.mjs`: **6.336 filas, ~88-90 días × 3 monedas**. ⚠️ La API trae **huecos** (50 h en SOL, 7 en BTC/ETH: horas sin liquidaciones que Coinalyze omite en vez de mandar a 0) — y la mediana rodante de producción hace `hist.slice(i-24, i)` **por posición, no por tiempo**, así que con huecos esa "ventana de 24 h" abarca algo más. Es propiedad del código ya desplegado, no la introduce esta serie; anotado para que nadie lo lea como un bug nuevo.
...
| `calculateATR(candles, period?)` | ATR Wilder — último elemento de `calculateATRSeries`, **pero REDONDEADO a 2 decimales** (3,87 vs 3,873507…): no pueden divergir en valor, sí en precisión. ⚠️ No rankear este escalar contra la serie sin redondear — el redondeo cae a un lado u otro según la ESCALA del activo (2026-08-01) |
...
| | |
|---|---|
| Código | **`v10_0_narrator`** · gate `g3_no_gates_narrator_pivot` · **574 tests backend** (32 suites) |
| Fase | **Pivot a ayudante de riesgo IMPLEMENTADO, VERIFICADO Y DESPLEGADO (2026-08-10).** El LLM deja de decidir/puntuar del todo — narra en prosa (6 secciones + resumen ejecutivo) — y el backend calcula `risk_geometry` (largo+corto SIMÉTRICOS, síncrono, sin depender de nada declarado por el LLM). Retirados enteros `gating.js`/`expectedScores.js`/`conditionalPlan.js`/`shadowTrade.js`/`decisionGates.js`/`analysisValidator.js` — sus columnas de BBDD quedan en NULL/0 en filas nuevas, filas anteriores intactas como histórico legible. Verificado con llamada real a Anthropic (BTC/Opus, luego ETH/SOL con Sonnet) de punta a punta: narrativa sin dictamen, geometría simétrica correcta, los 3 caminos de lectura (`POST /api/analyze`, `GET /api/history`, `GET /api/data`) contra la fila real en BBDD. Precede a esto: **Punto Cero 6 fase 1** (B1+F2+B4, 2026-08-09, sigue en producción sin cambios) y el **Bloque A+B** (~20 hipótesis direccionales, cero supervivientes, ver §REORIENTACIÓN abajo) que motivó el pivot. Recogida periódica sigue PAUSADA — el sistema opera bajo demanda; no se ha reactivado ningún cron |
| Producto | Lectura narrativa en prosa + geometría de riesgo simétrica (largo y corto a la vez, nunca elegidos) con 3 cifras medidas (R:R→equilibrio, `target_reachability_pct`) — sin dictamen direccional, sin plan condicional de una sola dirección, sin shadow trade |
| Producción | Pi `192.168.1.250:8080` · **3 monedas** (SOL/BTC/ETH), bajo demanda (sin cron activo) |
| Dónde mirar | **CLAUDE.md §REORIENTACIÓN** (abajo, motivo y medición) · `doc/BLUEPRINT.md` §"Pipeline de decisión — retirado" (arquitectura actual) · `backend/scripts/old/README.md` (scripts de auditoría archivados) |

### 🔄 REORIENTACIÓN del 2026-08-03 — qué cambió y por qué

El sistema llevaba **11 análisis y 11 `Esperar`**, no por prudencia sino **por construcción**
(hallazgo D6: el gatillo declarado y las puertas de score no coincidieron ni una vez, con
mecanismo). Se propuso un rediseño hacia un dictamen direccional continuo (`bias`) y **se midió
antes de escribirlo**. Dos NO-GO independientes lo archivaron:

- **Fase 0** (`auditDirectionalBias.mjs`): el bias determinista queda **por debajo del azar en
  3 de 3 monedas** y no supera a la deriva — perdiendo además **in-sample**.
- **M9** (`auditCleanMoveDirection.mjs`): **ninguna de 7 features pre-registradas** predice la
  dirección de un movimiento limpio; las que parecían replicar **cambiaron de signo en el
  contra-periodo**.

⚠️ **No significa que no haya señal:** el techo del oráculo sale **39,5-55,0 % contra un azar
...
**El producto resultante NO afirma ventaja direccional.** Este era el diseño original (plan
condicional de UNA dirección + `TRIGGER_BASE_RATE` + shadow trade); **implementado y
reemplazado el 2026-08-10** por algo más simple y más honesto con lo medido: el LLM narra en
prosa (sin `action`/`scores`/`conditional_setup`) y el backend calcula `risk_geometry` —
geometría de stop/objetivo **SIMÉTRICA** (largo Y corto a la vez, `utils/riskGeometry.js`,
nunca elegidos por el LLM ni por el backend) con `OPPORTUNITY_BY_HORIZON['24h']` (1×ATR stop /
2×ATR objetivo) y `target_reachability_pct` de la curva `TARGET_REACHABILITY` ya medida (en
los setups reales sale al **5-18 %** — o sea que lo normal es que el plan caduque abierto).
No hay plan condicional de una sola dirección ni gatillo pendiente: la entrada es siempre el
precio actual, así que `TRIGGER_BASE_RATE` y el shadow trade (`utils/shadowTrade.js`) dejaron
de tener sentido y se retiraron con el resto del pipeline de decisión — ver
`doc/BLUEPRINT.md` §"Pipeline de decisión — retirado" para el mapa completo de qué queda y
dónde.

**Se sigue negando a enseñar** `expectancy_r` (su línea base es una curva sin medir — M10) y
cualquier probabilidad de acierto direccional. `risk_score` sigue retirado; la convicción ya
no se pide al LLM en absoluto (no hay campo `conviction` en el output).

**Y no exigió punto cero:** es presentación/cálculo de cosas medibles desde datos que el
backend ya tenía (precio + ATR de decisión), sin re-entrenar ni re-medir nada.
...
- **Sprint Remediación Auditoría Red-Team (2026-07-07) — 17 hallazgos en 5 fases (PROMPT_VERSION v6_0 → v6_4)**. Auditoría externa "red team" del pipeline de decisión que identificó tres fallos estructurales: circularidad de validación, confluencia inflada por correlación/doble conteo, y backtesting no concluyente. Remediación por fases, cada una con tests y commit propio. **300 → 364 tests.** Herramienta de evidencia: `backend/scripts/auditStats.mjs` (read-only) cuantifica sesgo a Esperar, frecuencia de contradicciones/vetos, muestra real del backtest y fill-rate.
  - **C1 (temperatura)**: `analyzeMarket` enviaba a la API sin `temperature` → sampling por defecto del modelo → decisiones no reproducibles. **Corrección posterior (commit `b464100`):** los modelos actuales (Opus 4.8 / Sonnet 5, familia Claude 5) **DEPRECAN `temperature` — la Messages API responde `400` si se envía**, así que NO se puede fijar en 0. Por eso `env.analysisTemperature` es `null` por defecto y `buildLlmRequest` **omite** el campo (escape hatch `ANALYSIS_TEMPERATURE` reservado para un modelo futuro que sí lo acepte). **✅ CERRADO como LIMITACIÓN ACEPTADA (2026-08-02) — deja de ser deuda abierta.** Al no poder pasar `temperature`, la reproducibilidad NO está garantizada a nivel de API: los análisis corren con el sampling por defecto del modelo. **No tiene arreglo desde este lado** (es la API la que rechaza el campo), así que figurar como "pendiente" solo hacía que reapareciera en cada auditoría como si quedara trabajo. Está mitigado hasta donde la API permite. **Consecuencia que sí hay que recordar al leer el backtest:** dos análisis del mismo mercado pueden diferir por sampling, así que una diferencia pequeña entre dos observaciones **no es evidencia de que el mercado cambiara**. Reabrir solo si un modelo futuro vuelve a aceptar el campo (el escape hatch `ANALYSIS_TEMPERATURE` ya está puesto para ese día).
...
- **Prompt v8_0 — aprovechar el payload entero (2026-07-27, `v7_2` → `v8_0_full_payload`)** — auditoría del prompt cruzando las 651 claves del dataset contra las referencias del system. Resultado de partida: **0 referencias rotas** (el prompt no pedía ningún campo inexistente), pero varios bloques viajaban **sin dueño**.
  ...
  - **Reglas nuevas** (cada una consume un flag precalculado, nunca un número crudo):
    - **B4 · Régimen de volatilidad** — `bollinger_bands.volatility_state` (squeeze/normal/expansion) **calculado por terciles de su propia serie**: `width_pct` vale ~2,7 en 1h y ~33 en 1W para el mismo activo, así que un umbral absoluto habría sido otra constante disfrazada (fallo T5). ⚠️ **El "24-42 % por bucket en las 12 combinaciones moneda × TF" que se anotó aquí NO describía el reparto — describía el ruido del estimador (medido y corregido el 2026-08-01, `scripts/auditVolatilityState.mjs`).** Se rodó la ventana vela a vela, y dos ventanas consecutivas comparten `win−1` de sus `win` cierres: el n efectivo es ≈ `anclas/win` (13× menos en 1h), así que aquel IC salía ~13 veces demasiado estrecho y el 24-42 era el RANGO entre 36 celdas de un estimador ruidoso. Mismo fallo de denominador que el sesgo de 0,225R de la expectativa: un conjunto tratado como muestra independiente sin serlo. **Medido con anclas SIN SOLAPE: 35,9/31,3/32,8 % ±3,4 — las tres dentro del IC de 33,3 %, que es lo que exige la construcción** (`bucketByPercentile` corta en 0,33/0,67 sobre una muestra que incluye al propio valor). Tres matices que sí sobreviven: (1) **el bloqueo de fase es una trampa real** — muestrear 1h con paso de 168 velas son EXACTAMENTE 7 días y clava todas las anclas en la misma hora de la semana: BTC da **85,6 % `squeeze`**, y sobre los mismos bloques disjuntos con la fase derivando, **28,2 %**; producción está limpia (muestreo real a 08/20 UTC → 34,4/32,0/33,6, porque la estacionalidad es semanal y el cron recorre la semana entera); (2) **en 1W el estimador sí se desvía por geometría, no por mercado** — el nulo Monte Carlo con 1.500 caminos i.i.d. INDEPENDIENTES da 26,9 % `normal` / 37,3 % `expansion` (con `win=52` la serie de anchuras tiene 33 valores solapados y el rango del último deja de ser uniforme); 1h/4h/1D pasan; (3) la concentración del rango en los extremos también la reproduce el nulo → es geometría, y significa que un `squeeze` suele ser PROFUNDO, no uno que roza el corte. La regla clave: **un squeeze no tiene dirección**, favorece `Preparar` con dos escenarios, no `Comprar`.
