...
src/services/                ← Lógica de negocio, I/O externo, cache
  ...
  (scripts) auditPriceBand.mjs ← **Resultado: la banda está BIEN puesta (2026-07-30)**. Nació de que los dos primeros análisis de `v9_0` salieran con `oi_price_cell: "no_signal"` — el segundo con el eje de OI VIVO (+1,68 %) pero el precio dentro de banda (−0,81 % vs ~1,6 %), a UNA celda del primer `Vender` (volume y structure ya en −1). Mide la FUGA (a 0,50× se silencia el **55 %** de los anclajes con OI vivo) y, sobre todo, si el lift SOBREVIVE al aflojar: **`0,50×` es la única banda con las seis celdas positivas** (2 celdas × 3 monedas) y donde `OI↓px↑` alcanza su máximo consistente (+15,0/+6,7/+5,8); a 0,75× y 1,00× se va a NEGATIVO en BTC y ETH. Apretar a 0,35× bajaría la fuga al 41 % pero el lift de `OI↓` cae a menos de la mitad — en señal total pierde. **La fuga no es un defecto, es la conjunción**: el OI no tiene dirección propia y el precio es competencia de Structure, así que el eje solo puntúa cuando ambos hablan. Techo estructural: 64 % eje vivo × ~45 % precio fuera de banda ≈ 29 % con celda, que cuadra con el 28,9 % medido y con el `\|score\|>=1` del 23-25 %
  (scripts) auditDvolRegime.mjs ← **Resultado NEGATIVO documentado (2026-07-29)**: el DVOL de BTC NO merece regla. Medido sobre 365 días × SOL/ETH/BTC (n=2.170 anclajes 4h): `elevated`+`panic` suman **12 anclajes (0,3 %)** —el índice pasó el año entre 33,8 y 48,3 con mediana 42,8, así que los cortes en 60/80 son ramas muertas— y los dos buckets vivos dan lift de ±1 punto **con el signo invertido entre monedas** (ruido). Lo único real: la implícita SÍ predice la realizada (ATR% 1,90→2,38→2,34→5,73 en SOL, monótono en las tres). Pero eso ya lo mide el `ATR%` del propio activo, que además normaliza todos los umbrales → sería doble conteo con un proxy indirecto y con retardo. **Nota de método:** la métrica de oportunidad está ATR-normalizada, así que era incapaz de detectar una señal de volatilidad por construcción — la pregunta estaba mal planteada. Revisar solo si el mercado entra en DVOL > 60 sostenido, y entonces por la puerta de RIESGO (tamaño de posición), no la direccional
...
src/utils/
  ...
  stats.js                   ← IC de Wilson + **Fase 5**: `classifyOpportunity` (¿ofreció el mercado un movimiento operable? = `targetK`×ATR antes de `adverseK`×ATR en contra, SIN dirección propia), `classifyPathOutcome` (win-rate path-aware: mira el camino, no el destino), `maxExcursionAtr`, `convictionBucket`, y los agregadores `summarizeOpportunity`/`summarizePathWinRate`/`summarizeConviction`. `OPPORTUNITY_DEFAULTS` (2×/1×ATR) son CONVENCIONES sin calibrar — viajan en la respuesta (`thresholds`) para que la cifra nunca se lea sin su regla. `MIN_DIRECTIONAL_SAMPLE=20` vive aquí para que el win-rate clásico y el path-aware compartan gate
