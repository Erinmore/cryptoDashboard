...
**`GET /api/analyze/payload` añade además:** `order_book` (muros + imbalance), `derivatives.liquidation_clusters` (magnetic zones inferidas + flags `magnetic_long/short_zone_active`), `onchain` (MVRV/NUPL/SOPR para BTC), `etf_flows` (BTC/ETH spot ETF flows vía SoSoValue), `macro` (DXY/SPX/Gold vía Yahoo Finance), `volatility` (btc_dvol/eth_dvol via Deribit — value, regime, change_24h_pct), `volume_history` (CVD/VWAP), `timeframe_analysis` (conflictos entre TFs), **`gating`** (vetos deterministas `veto_long/veto_short/veto_reason` + `contradictions[]`/`contradiction_count` (por bloques)/`contradiction_blocks[]` precalculados por `utils/gating.js`), resúmenes consolidados de históricos. `technical[tf]` incluye además `volume_profile` (con `price_vs_poc`, `excursion`) y `smc` (BOS/CHoCH/FVGs con `signal_status`); `cvd.cvd_strength` y `vwap.price_vs_vwap` también precalculados.
...
- **372 tests totales** (tras el sprint de remediación de auditoría + seguimiento): `indicators.test.js` + `integration.test.js` + `analysisValidator.test.js` + `outcome.test.js` + `outcomeService.test.js` + `gating.test.js` + `decisionGates.test.js` + `expectedScores.test.js` + `stats.test.js` + `timeSeries.test.js` + `timeframeConflicts.test.js` + `levelStrength.test.js` + `historyPersistence.test.js` + `extractJson.test.js` + `fundingSummary.test.js` + `modelSelection.test.js` + `lsrSummary.test.js`
...
- **Seguimiento revisión crítica (2026-07-07, PROMPT_VERSION v6_4 → v6_5_block_dedup)** — 3 hallazgos de una relectura interna previa a la 2ª auditoría. 364 → 372 tests.
